...
当前人格积分主要发生在 event step。普通事件已被强力降权，但高影响事件序列的变化速度仍与事件输入次数有关。未来应让 elapsed time、事件密度和 recovery window 成为明确输入，避免“记录得越勤，人格变化越快”。
...
2. 增加时间尺度不变性测试：相同事件在一天内密集发生与分散在数月发生，应由 recovery/decay 产生可解释差异。
